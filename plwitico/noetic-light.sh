...
ACTION=
if [ $# -eq 0 ] ; then
   # If no options are specified at all, use the name "plwitico-noetic-light"
   CONTAINER_NAME=plwitico-noetic-light
else
  case "$1" in
    rm|remove)
      shift
  ...
  CONTAINER_NAME="${CONTAINER_NAME:-plwitico-noetic-light}"
fi
...
if [ -n "${CONTAINER_NAME:-}" ] ; then
    if [ -z "$($DOCKER_EXECUTABLE ps -aq --filter name=^$CONTAINER_NAME\$)" ] ; then
       ...
       if [ "$(count_positional_args $@)" == "0" ] ; then
          ...
          DUMMY_DEFAULTS="-t -d ssilenzi/plwitico:noetic-light"
       fi
    else
...
docker cp "$(dirname ""$0"")/noetic-init.sh" $CONTAINER_NAME:/workspace/
docker exec -it $CONTAINER_NAME bash -i -c "sudo chown ubuntu:ubuntu /workspace/noetic-init.sh; \
