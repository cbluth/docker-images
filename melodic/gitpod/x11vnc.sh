...
apt update 
apt install -y libxtst-dev libssl1.0-dev libjpeg-dev libtcl8.6 libtk8.6 \
  libvncclient1 libvncserver1 tcl tcl8.6 tk tk8.6 zlib1g-dev x11vnc \
  x11vnc-data
apt purge -y x11vnc x11vnc-data
...
# Clean.
cd ..
apt install -y --autoremove --purge libssl-dev libtcl8.6 libtk8.6 \
